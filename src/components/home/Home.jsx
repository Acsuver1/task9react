...
const Home = () => {
    ...
    return (
        <>
                ...
                <div className="container mx-auto max-w-[1440px] p-5 flex justify-center items-center">
                    <div className="w-full">
                        <form className="relative">
                            <input type="text" value={searchText} onChange={handleSearchChange} className="max-w-[400px] py-2 pl-10 pr-4 text-gray-700 bg-white border border-red-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-d-500 focus:border-blue-500  font-jakarta" placeholder="Search " />
            ...
            <section>
                <div className="container mx-auto max-w-[1440px] p-5 bg-gray-300 p-[25px]  rounded-lg">
                    <div className="home__wrapper flex flex-wrap  justify-center gap-5 font-sans   " >
                        {recipes && recipes.length > 0 ? (
                            recipes.map((recipe) => (
                                <Link key={recipe.id} to={`/recipe/${recipe.id}`}>
                                    <div className="w-[453px] h-[350px] recipe-card bg-gray-100 rounded-lg shadow-md overflow-hidden transition-transform transform hover:scale-105 ">
                                        <img className="recipe-image w-full h-40 object-cover transition-transform duration-100 transform hover:scale-105" src={recipe.image} />
